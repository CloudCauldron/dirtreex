...
%%
%% Clean build (no .aux/.toc): run lualatex dirtreex_examples.tex three times
%% to settle the table of contents and tree-location references.
%% With settled auxiliary files and unchanged layout, one run normally suffices.
...
The \verb|dotfill color| and \verb|dotfill command| options customize the
leader between each entry name and its comment. They apply to the current
tree, including its root and nested entries. The defaults are the current
text colour and \verb|\dotfill|.

\subsection{Coloured dots}

Only the leader takes the selected colour. Empty comments have no leader,
and explicit \verb|\\| continuations keep their right alignment.

\begin{Verbatim}
\begin{dirtreex}[dotfill color=blue!60]
  \dir{project}{root directory}{
    \verbfile|main_app.py|{entry point}
    \dir{docs}{documentation}{
      \file{guide.txt}{first line\\second line}
    }
    \file{LICENSE}{}
  }
\end{dirtreex}
...
\subsection{Solid leaders}

Use the standard \LaTeX{} command \verb|\hrulefill| for a solid rule.
The tree connectors can be styled independently.

\begin{Verbatim}
\begin{dirtreex}[
  dotfill color=teal, dotfill command=\hrulefill,
  line color=gray]
  \archive{release.zip}{distribution}{
    \file{app.py}{application}
...
\subsection{Custom dashed leaders}

A custom command can repeat a small box with \verb|\cleaders| and stretch
it with \verb|\hfill|. Here each \verb|0.8em| box contains a centred
\verb|0.4em| dash. The final \verb|\kern0pt| follows the standard
\LaTeX{} fill commands. Dimensions are evaluated in the tree's font.

\begin{Verbatim}
\begin{dirtreex}[
  dotfill color=purple,
  dotfill command={%
    \cleaders\hbox to 0.8em{%
      \hss\rule[0.4ex]{0.4em}{0.4pt}\hss}%
    \hfill\kern0pt}]
  \dir{project}{custom dashed leaders}{
    \file{main.py}{entry point}
    \dir{data}{resources}{
      \file{sample.csv}{sample data}
    }
  }
\end{dirtreex}
\end{Verbatim}
...
For a blank gap, use \verb|dotfill command=\hfill|. Custom commands should
provide \verb|\hfill| stretch and stay within the normal text height.
Declarations in the command are scoped to the leader.
...
Put a symbol or image directly in \verb|icon={...}|, as in this small tree:

\begin{Verbatim}
\begin{dirtreex}
  \dir[icon={+}]{project}{root}{
    \verbfile[icon={\includegraphics[height=1.5ex]{assets/python.png}}]
...
\subsection{Per-entry icons}

Use \verb|icon={...}| to choose an icon for an individual entry. The option
works with both normal and verbatim names. The default name formatter adds
a nonbreaking \verb|0.5em| gap after each icon; entries without an icon keep
their usual spacing. Font and colour declarations inside an icon are local.

The PNG icons use \verb|\includegraphics| for sizing and \verb|\raisebox| for
baseline placement. The files are in \verb|assets/|, and the \verb|!| prefix
is ordinary TeX content.

Here, \verb|\entryicon| simply avoids repeating the image-sizing code. The outer group keeps that helper definition local to this example.

\begin{Verbatim}
\begingroup
\newcommand{\entryicon}[1]{%
  \raisebox{-.15em}{\includegraphics[height=.9em]{assets/#1}}}
\begin{dirtreex}[dotfill color=gray]
  \verbdir[icon={\entryicon{directory.png}}]|project_$STAGE|{root}{
    \dir[icon={\entryicon{directory.png}}]{src}{source code}{
      \verbfile[icon={\entryicon{python.png}}]|main_app.py|{entry point}
      \file[icon={\color{red}\bfseries !}]{TODO.txt}{review needed}
    }
    \archive[icon={\entryicon{zip_file.png}}]{data.zip}{resources}{
      \verbfile|sample_data.csv|{no icon inherited}
    }
    \file{LICENSE}{no icon}
  }
\end{dirtreex}
\endgroup
\end{Verbatim}
...
An explicit \verb|icon={}| also adds no prefix or gap. Each icon is stored
without expansion and is rendered in the tree's font. Size and position
image icons to fit the normal text height and depth.

\clearpage

\subsection{Automatic icons with \texttt{\textbackslash DirtreexFormatName}}
...
This hook replaces the complete name formatter, including per-entry icon
handling. To keep that handling when decorating names, save and call the
original \verb|\DirtreexFormatName|. For full control,
\verb|\DirtreexGetField{#1}{i}| retrieves the raw icon code.
