...
These examples use the verbatim variants \verb|\verbdir| / \verb|\verbfile|. The path argument is read under xparse's \verb|v| specifier; catcodes inside the chosen delimiters are set to 12 (``other''), so common dangerous bytes survive as literal glyphs.
...
\verb|\dir| / \verb|\file| (normal-catcode names) and \verb|\verbdir| / \verb|\verbfile| (verbatim names) coexist freely inside the same children block.
